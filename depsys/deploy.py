...
@socketio.on('executing', namespace=namespace)
...
@socketio.on('batch_executing', namespace=namespace)
def batch_exec():
    """Func for batch deploy"""
    room = batch_socket_room
    join_room(room)
    emit('my_response', {'data': "Projects version detail refer to file: " + extra_args_file +
                                 ", which post in " + SystemConfig().get().repository_server,
                         'time_stamp': "\n" + time.strftime("%Y-%m-%d:%H:%M:%S", time.localtime()) + ":"},
         room=room)
    try:
...
def execute_thread(info, single=True):
    ...
    # time_end will be overwrite, just set an initial value here
    ...
    try:
        ...
        app.logger.info('Removed %s' % str(working_path))
    time.sleep(2)
    try:
        # get deploy package
    ...
    ansible_bin = SystemConfig().get().ansible_path.strip()
    # command is like: /usr/bin/ansible-playbook -i my_host_file my_playbook.yml
    ...
    with open(logs_file) as logs:
        ...
        while running:
            ...
            if output and single:
                socketio.emit('my_response', {'time_stamp': "", 'data': output}, namespace=namespace, room=room)
            ...
            if proc.poll() is not None:
                ...
                socketio.emit('my_response',
                              {'time_stamp': "\n" + time.strftime("%Y-%m-%d:%H:%M:%S", time.localtime()) + ":",
                               'data': "[" + project + "] 发布脚本执行完毕!"},
                              namespace=namespace, room=room)
    # write logs into record
    with open(logs_file) as logs:
        logs = logs.read()
        # change return code for status,
        # 0 for successful termination, >0 for termination with an error code, <0 if was killed
        returncode = proc.poll()
        if returncode == 0:
            status = 1
            socketio.emit('my_response',
                          {'data': "[" + project + "] 发布成功，请检查进程是否正常！",
                           'time_stamp': "\n" + time.strftime("%Y-%m-%d:%H:%M:%S", time.localtime()) + ":"},
                          namespace=namespace, room=room)
        ...
        if os.path.isfile(extra_file):
            with open(extra_file, encoding='utf-8') as release_info:
                ...
                try:
                    ...
                    socketio.emit('my_response',
                                  {'data': "[" + project + "] Read " + extra_args_file + " failed due to: " + str(Err) + "\n",
                                   'time_stamp': "\n" + time.strftime("%Y-%m-%d:%H:%M:%S", time.localtime()) + ":"},
                                  namespace=namespace, room=room)
                    app.logger.error(str(Err))
                    DeployRecord().add(project=project, status=status, version=branch,
                                       requester="N/A", deploy_reason="N/A", deployer=deployer,
                    ...
                    for info in release_info['发布列表']:
                        if info['发布版本'] == branch:
                            ...
                            DeployRecord().add(project=project, status=status, version=branch,
                                               requester=requester, deploy_reason=deploy_reason, deployer=deployer,
                                               time_begin=time_begin, time_end=time_end, pkg_md5=pkg_md5, logs=logs)
        else:
            DeployRecord().add(project=project, status=status, version=branch,
                               requester="N/A", deploy_reason="N/A", deployer=deployer,
...
def get_playbook(package_name, local_package):
    ...
    # replace the template playbook content to current deployment
...
def get_package(project, version, room):
    ...
    if username and password:
        auth_info = urllib.parse.quote(username) + ":" + urllib.parse.quote(password)
    ...
    if os.path.isfile(deployed_package):
        socketio.emit('my_response',
                      {'data': "[" + project + "] Found deployed local package, use it for this deployment." + "\n",
                       'time_stamp': "\n" + time.strftime("%Y-%m-%d:%H:%M:%S", time.localtime()) + ":"},
                      namespace=namespace, room=room)
    ...
    # get package from remote git
        ...
        if conf.source_address:
            ...
            try:
                ...
                socketio.emit('my_response',
                              {'data': "[" + project + "] Downloading package from gitlab..." + "\n",
                               'time_stamp': "\n" + time.strftime("%Y-%m-%d:%H:%M:%S", time.localtime()) + ":"},
                              namespace=namespace, room=room)
                my_remote.pull(version)
            except Exception as Err:
                socketio.emit('my_response',
                              {'data': "[ERROR] [" + project + "] Download package from repository failed, please check your setting! \n",
                               'time_stamp': "\n" + time.strftime("%Y-%m-%d:%H:%M:%S", time.localtime()) + ":"},
                              namespace=namespace, room=room)
                app.logger.error(str(Err))
        ...
        # use git sparse checkout to get specific files
            ...
            os.chdir(str(project_work_path(str(project))))
            shell_command = 'git config core.sparseCheckout true && echo "' + version\
            ...
            try:
                ...
                if os.path.isfile(package):
                    ...
                    socketio.emit('my_response',
                                  {'data': "[ERROR] [" + project + "] Seems this version doesnt' include a package, please check!" + "\n",
                                   'time_stamp': "\n" + time.strftime("%Y-%m-%d:%H:%M:%S", time.localtime()) + ":"},
                                  namespace=namespace, room=room)
...
def get_branches(room):
    ...
    shell_command = 'git config core.sparseCheckout true && echo "'\
                    + extra_args_file + '" >> .git/info/sparse-checkout'
    subprocess.Popen(shell_command, shell=True)
    # pull specific path via gitPython
    try:
        ...
        socketio.emit('my_response',
                      {'data': "Downloading " + extra_args_file + " from repository...",
                       'time_stamp': "\n" + time.strftime("%Y-%m-%d:%H:%M:%S", time.localtime()) + ":"},
                      namespace=namespace, room=room)
        my_remote.pull('master')
    except Exception as Err:
        socketio.emit('my_response',
                      {'data': "[ERROR] Download " + extra_args_file + " from repository failed, please check your setting!",
                       'time_stamp': "\n" + time.strftime("%Y-%m-%d:%H:%M:%S", time.localtime()) + ":"},
                      namespace=namespace, room=room)
        app.logger.error(str(Err))
        ...
        socketio.emit('my_response',
                      {'data': "Saved " + extra_args_file + " to " + str(working_path),
                       'time_stamp': "\n" + time.strftime("%Y-%m-%d:%H:%M:%S", time.localtime()) + ":"},
                      namespace=namespace, room=room)
        with open(extra_args_file, encoding='utf-8') as release_info:
            ...
            try:
                ...
                socketio.emit('my_response',
                              {'data': "Read " + extra_args_file + " failed due to: " + str(Err),
                               'time_stamp': "\n" + time.strftime("%Y-%m-%d:%H:%M:%S", time.localtime()) + ":"},
                              namespace=namespace, room=room)
                app.logger.error(str(Err))
                ...
                # sample of batch_list
                # ['isp_finance@20180820_1500', 'isp-forward@20180820_1501', 'isp_web@20180820_1502']
                return batch_list
